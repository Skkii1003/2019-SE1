...
public class ClassLoader {
    ...
    private void resolveInterfaces(JClass clazz) throws ClassNotFoundException {
        ...
        for(int i=0;i< clazz.getInterfaceNames().length;i++){
            interfaces[i] = loadClass(clazz.getInterfaceNames()[i], clazz.getLoadEntryType());
        }
    }
    ...
    private void loadValueFromRTCP(JClass clazz, Field field) {
        ...
        if(constantpoolindex > 0){
            switch(field.getDescriptor()){
                ...
                case "S":
                case "I":
                    staticvars.setInt(slotID,((IntWrapper) runtimeConstantPool.getConstant(constantpoolindex)).getValue());
                    break;
                case "J":
                    long longval = ((LongWrapper) runtimeConstantPool.getConstant(constantpoolindex)).getValue();
